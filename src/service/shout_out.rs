...
impl ShoutOutService {
    ...
    pub async fn shout_out(&self, user: &str, req: ShoutOutRequest) -> Result<()> {
        ...
        for (_, output) in psbt.unsigned_tx.output.iter().enumerate() {
            let output_addr = bitcoin::Address::from_script(&output.script_pubkey, CONFIG.bitcoin_network()).map_err(|e| AppError::BadRequest(e.to_string()))?.to_string();
        ...
        let db_listing = self
            ...
            .await?
            .ok_or_else(|| {
                AppError::BadRequest(format!("Listing for '{}' not found", req.name))
            })?;
        if db_listing.seller_address != user {
            return Err(AppError::Forbidden("The listing of the name is not yours".to_string()));
        }
        let psbt_base64 = base64::engine::general_purpose::STANDARD.encode(psbt.serialize().as_slice());
        ...
        loop {
            ...
            if let Ok(pending_shout_outs) = self.postgres.get_pending_shout_out().await {
                for shout_out in pending_shout_outs {
                    if let Ok(Some(confirms)) = self.blockchain.get_transaction_confirmations(shout_out.tx_id.as_str()).await {
                        if confirms > 0 {
                            let _ = self.postgres.confirm_shout_out(shout_out.tx_id.as_str()).await;
                        }else {
