...
use crate::domain::{
    GetPoolRequest, GetPoolResponse, ListNameParams, ListNameRequest, ListNameResponse,
    ListingInfo, ListingsResponse,
};
use crate::error::{AppError, Result};
use crate::infra::orchestrator_canister::InvokeArgs;
use crate::infra::{DynBlockchainClient, DynPostgresClient, DynRedisClient, IcAgent, ListingMeta};
...
impl TradingService {
    ...
    ///
    /// Requires the caller's address to verify ownership.
    ...
    pub async fn get_pool(
        ...
        // Verify ownership: check if name belongs to caller's address
        let ord_data = self.blockchain.ord_bns_address(caller_address).await?;

        // Find the name in user's names
        let name_entry = ord_data
            .runes
            .iter()
            .find(|r| r.rune_name == request.name)
            .ok_or_else(|| {
                AppError::Forbidden(format!(
                    "Name '{}' is not owned by address {}",
                    request.name, caller_address
                ))
            })?;
        ...
        let pool_address = self.ic_agent.create_pool(&request.name).await?;
    ...
    pub async fn list_name(&self, request: &ListNameRequest) -> Result<ListNameResponse> {
        ...
        // Store tx_id in database for tracking
